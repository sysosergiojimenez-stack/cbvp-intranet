...
import { exportarMiAsistenciaPdf } from '@/lib/exportarInformePdf';
...
export default function MiDashboard() {
  ...
  const [exportandoAsistencia, setExportandoAsistencia] = useState(false);
  const handleExportarAsistencia = async () => {
    setExportandoAsistencia(true);
    try {
      await exportarMiAsistenciaPdf({
        mes: mesAsistencia,
        anio: anioAsistencia,
        categoria: categoriaAsistencia as 'COMBATIENTE' | 'ACTIVO',
        diasDelMes: dataAsistencia?.diasDelMes || 0,
        normales: misNormales,
        especiales: misEspeciales,
        practicas: misPracticas,
        sabados: dataPC?.sabados || [],
        citaciones: misCitaciones,
        fechasCitacion: dataPC?.fechasCitacion || [],
        sinCitaciones: !!dataPC?.sinCitaciones,
        totalAcumulado: miTotal,
      });
    } finally {
      setExportandoAsistencia(false);
    }
  };
      ...
      <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-6">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-3">
          ...
          <button
            ...
            disabled={exportandoAsistencia || cargandoAsistencia || !categoriaValida || (misNormales.length === 0 && misEspeciales.length === 0)}
          ...
          >
            <Download className="w-4 h-4" /> {exportandoAsistencia ? 'Generando...' : 'Exportar PDF'}
          </button>
        </div>
        <div className="flex flex-wrap items-center gap-3 mb-4 text-[11px] text-white/40">
