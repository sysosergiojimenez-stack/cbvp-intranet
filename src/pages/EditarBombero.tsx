...
export default function EditarBombero() {
  ...
  useEffect(() => {
    if (data?.exito && data.bombero) {
      ...
      setForm({
        codigo: b.codigo,
        anioJuramento: b.anioJuramento,
        categoria: b.categoria || 'COMBATIENTE',
        rango: b.rango || 'Voluntario(a)',
        codigoRadial: b.codigoRadial,
  ...
  const editarMutation = trpc.personal.editar.useMutation({
    onSuccess: (res) => {
      if (res.exito) {
        setSuccess('Bombero actualizado correctamente');
        setError('');
  ...
  if (isLoading) {
    return <div className="text-white/60 text-center py-12">Cargando datos del bombero...</div>;
  }
